...
# Need to get MRGID and Gazetter name ----
# loop through both datasets 
iho_info <- data.frame()
nations_info <- data.frame()

for (placename in unique(iho$name) %>% sort()) {
  
  print(placename)
  
  try(
    temp_info <- mr_geo_code(place = placename))
  
  iho_info <- bind_rows(iho_info, temp_info)
  
}

for (placename in unique(nations$territory) %>% sort()) {
  
  print(placename)
  
  try(temp_info <- mr_geo_code(place = placename))
  
  nations_info <- bind_rows(nations_info, temp_info)
  
}

iho_info_clean <- iho_info %>%
  dplyr::distinct() %>%
  dplyr::filter(placeType %in% "IHO Sea Area") %>%
  dplyr::select(MRGID, placeType, preferredGazetteerName) %>%
  glimpse()

nations_info_clean <- nations_info %>%
  dplyr::distinct() %>%
  dplyr::filter(placeType %in% "Nation") %>%
  dplyr::filter(gazetteerSource %in% "(2001). The Times comprehensive atlas of the world. 10th ed. Times Books: London. ISBN 0-7230-0792-6. 67, 220, 124 plates pp.") %>%
  dplyr::select(MRGID, placeType, preferredGazetteerName) %>%
  glimpse()

unique(iho_info$placeType) %>% sort()


# for (url in unique(test.distributions$higherGeographyID)) {
#   print(url)
#   
...
#   
#   # Extract the text between "placeType" and "latitude"
#   extracted_text <- regmatches(full_text, gregexpr('placeType(.*?)latitude', full_text, ignore.case = TRUE))
#   
#   # Print the extracted text
#   extracted_text <- extracted_text[[1]]
#   extracted_text
#   
#   # Remove "placeType\":\"" and "\",\"latitude"
#   cleaned_string <- gsub("placeType\":\"|\",\"latitude", "", extracted_text)
#   
#   new.dat <- data.frame(url = url, 
#                     shapefile = cleaned_string)
#   
#   dat <- bind_rows(dat, new.dat)
# }
# 
# # URL of the page to scrape
# url <- "http://marineregions.org/mrgid/7963"
# 
# # Read the HTML content of the page
# page <- read_html(url)
# 
