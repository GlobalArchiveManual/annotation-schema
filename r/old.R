...
#   distinct(Genus_species) # we should be getting a and b for everything - not just the ones that are missing BG 27/02

work.dir<-"C:/Users/21301367/Google Drive/Work/Analysis_GlobalArchive_Australia/Life_history"
setwd(work.dir)


##### Bring in life-history sheets from around AUs and then choose one ----

dir()
vic<-read_tsv("08_LifeHistorySpeciesList.txt")%>%
  rename(Scientific=Genus_species)%>%filter(!is.na(a))%>%filter(!is.na(Length.measure))%>%
  select(Scientific,Length.measure,a,b,aLL,bLL,Source)%>%
  mutate(a=as.numeric(a),b=as.numeric(b),aLL=as.numeric(aLL),bLL=as.numeric(bLL))%>%
  mutate(LH="vic LH sheet")

str(vic)
names(vic)

nsw<-read_csv("NSW_life_history_190118.csv")%>%
  rename(Scientific=Genus_species)%>%
  select(Scientific,Length.measure,a,b,aLL,bLL,Source)%>%
  filter(!is.na(a))%>%
  mutate(a=as.numeric(a),b=as.numeric(b),aLL=as.numeric(aLL),bLL=as.numeric(bLL))%>%
  mutate(LH="nsw LH sheet")

qld<-read_csv("seq_min_size.csv")%>%
  rename(Scientific=sp,a=lw_a,b=lw_b,Length.measure=lw_corr_type)%>%
  filter(!genus==species)%>%
  filter(!species%in%c("sp","sp1","sp2"))%>%
  mutate(Length.measure=ifelse(Length.measure==1,"FL",
                               ifelse(Length.measure==2,"TL",
                                      ifelse(Length.measure==3,"Unknown",Length.measure))))%>%
  select(Scientific,Length.measure,a,b)%>%
  mutate(a=as.numeric(a),b=as.numeric(b))%>%
  mutate(LH="qld LH sheet")%>%
  filter(Length.measure=="FL")%>%
  mutate(aLL=0)%>%
  mutate(bLL=1)

wa<-gs_title("Life_history")%>%
  gs_read_csv(ws = "Life_history")%>%
  rename(Scientific=Genus_species)%>%
  select(Scientific,Length.measure,a,b,aLL,bLL,Source)%>%
  filter(!is.na(a))%>%mutate(a=as.numeric(a))%>%
  #mutate(test=a*1000)%>%
  mutate(a=as.numeric(a),b=as.numeric(b),aLL=as.numeric(aLL),bLL=as.numeric(bLL))%>%
  mutate(LH="wa LH sheet")

combined<-bind_rows(vic,nsw,qld,wa)%>%
  arrange(Scientific,LH)%>%
  filter(!Length.measure%in%c(NA,"??","Other","0"))%>%
  separate(Scientific, into = c("Genus","Species")," ",extra = "merge")%>%
  filter(!Species%in%c(NA,"sp1","sp","sp 1","sp Broome","sp1","sp10","sp2","spp"))%>%
...
names(combined.distinct)

## Change any synonyms in this sheet to correct then to fishbase to match up----
synonyms.cor <- gs_title("Synonyms_Australia")%>%#register a sheet
  gs_read_csv(ws = "Synonyms_Australia")%>%distinct()

# Bring in synonyms to change to fishbase names ----
synonyms.fb <- gs_title("Synonyms_Australia")%>%#register a sheet
  gs_read_csv(ws = "fishbase_names")%>%
  select(-Comment)%>%
  dplyr::rename(Family=LH.Family, Genus=LH.Genus, Species=LH.Species)%>%
  filter(!is.na(FB.Species)) # filters out the four that are not it fishbase

# First change any old names in life-history sheets ----
combined.distinct<-left_join(combined.distinct,synonyms.cor,by=c("Genus","Species"))%>%
...
# Then Chage to FB names----
# combined.distinct<-left_join(combined.distinct,synonyms.fb,by=c("Family","Genus","Species"))%>%
#   mutate(Family=ifelse(!is.na(FB.Family),FB.Family,Family))%>%
#   mutate(Genus=ifelse(!is.na(FB.Genus),FB.Genus,Genus))%>%
#   mutate(Species=ifelse(!is.na(FB.Species),FB.Species,Species))%>%
#   select(-c(FB.Family,FB.Genus,FB.Species))%>%
#   mutate(Scientific=paste(Genus,Species,sep=" "))
...
not.validated = as.data.frame(setdiff(listforFB, validatedlist))%>%dplyr::rename(species="setdiff(listforFB, validatedlist)") # 45 species do not match - buuut fishbase can find a synonym for most of them there are just 5 species where there is no match

# Used this for endemism 
...
has.lw = unique(as.character(LW$Scientific)) # 692
no.lw = as.data.frame(setdiff(validatedlist, has.lw)) # 908
write.csv(no.lw, paste(name, "no.lw.csv", sep = "."), row.names = FALSE)


# Clean up the FishBase data and join with country codes----
clean.info = LW%>%#change to LW
...
#List and summarise Max lengths and create test length for all species
LH.lengths<-LH%>%
...
### Change to Life-history names ----
lw.ll.top<-left_join(lw.ll.top,synonyms.cor,by=c("Genus","Species"))%>%
  mutate(Genus=ifelse(!is.na(Genus_correct),Genus_correct,Genus))%>%
  mutate(Species=ifelse(!is.na(Species_correct),Species_correct,Species))%>%
  select(-c(Family,Family_correct,Genus_correct,Species_correct))%>%
  mutate(Scientific=paste(Genus,Species,sep=" "))%>%
  distinct()

# read in equations from Life History sheet and add
LHist.edit<-combined.distinct%>%#LHist%>%
  rename(Type_LH=Length.measure,Where.obtained=LH)%>%
  select(Scientific,Type_LH,a,b,aLL,bLL,Where.obtained,Source)%>%
  rename(a_LH=a)%>%
  rename(b_LH=b)%>%
  rename(aLL_LH=aLL)%>%
  rename(bLL_LH=bLL)
...
complete.fb.records<-lw.ll.top%>% # 692 species
  #filter(!is.na(Type))%>%
  filter(!is.na(a))%>%
  filter(!is.na(b))%>%
  #filter(!is.na(aLL))%>%
  #filter(!is.na(bLL))%>%
  rename(Length.measure=Type)%>%
  mutate(Source="FishBase")%>%mutate(Source_Tax_Level="Species specific")%>%
  select(Scientific,Length.measure,a,b,aLL,bLL,Source,Source_Tax_Level)%>%
  separate(Scientific,into = c("Genus","Species"),extra = "merge")

names(complete.fb.records)

## Change any synonyms ----

complete.fb.records<-left_join(complete.fb.records,synonyms.cor,by=c("Genus","Species"))%>%
  mutate(Genus=ifelse(!is.na(Genus_correct),Genus_correct,Genus))%>%
  mutate(Species=ifelse(!is.na(Species_correct),Species_correct,Species))%>%
  select(-c(Family_correct,Genus_correct,Species_correct,Family,Comment))%>%
  mutate(Scientific=paste(Genus,Species,sep=" "))
## Replace current life-history where all FB values are available ----
names(LH)
LH<-LH%>%
  select(-c(Length.measure,a,b,aLL,bLL,Source))

new.LH<-left_join(LH,complete.fb.records,by="Scientific")





write.csv(new.LH,"new.LH.csv")
missing.from.fb<-anti_join(lw.ll.FB_LH, complete.fb.records, by="Scientific") #719
names(lw.ll.FB_LH)


test<-gs_new("Australia.life.history",ws_title = "australia.life.history",input=new.LH)
